...
>
    <div 
    className="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] shadow-2xl flex flex-col" 
    ...
    <div className="relative flex-shrink-0">
        ...
        <button
        onClick={onClose}
        className="absolute top-4 right-4 bg-white rounded-full p-2 hover:bg-gray-100 shadow"
        >
        <X className="w-6 h-6" />
        </button>
    </div>
    ...
    <div className="overflow-y-auto flex-1">
        ...
        {project.titlegallery && project.titlegallery.length > 0 && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div key={index} className="flex flex-col items-center">
                    <img 
                    src={imgSrc} 
                    alt={project.titlegallery ? project.titlegallery[index] : `Gallery image ${index + 1}`} 
                    className="w-full h-48 object-cover rounded-lg mb-2" 
                    />
                    {project.titlegallery && project.titlegallery[index] && (
                    <span className="text-sm text-gray-600">{project.titlegallery[index]}</span>
                    )}
                </div>
                ))}
            </div>
